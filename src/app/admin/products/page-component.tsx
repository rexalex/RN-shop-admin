...
// TODO alternative to using FC for props
type Props = {
  categories: Category[];
...
export const ProductPageComponent: FC<Props> = ({
  ...
  const productCreateUpdateHandler = async (
    ...
    if (images.length > 0) {
      const imagesPromise = Array.from(images).map((file) =>
        uploadFile(file as File)
      );
      try {
        imagesUrls = (await Promise.all(imagesPromise)) as string[];
      } catch (error) {
      ...
      <div className="container mx-auto p-4">
          ...
          <TableBody>
            {productsWithCategories.map((product) => (
              <ProductTableRow
                ...
                setIsDeleteModalOpen={(ev) => setIsDeleteModalOpen(ev)}
              />
            ))}
          </TableBody>
        </Table>
        ...
        <Dialog open={isDeleteModalOpen} onOpenChange={setIsDeleteModalOpen}>
          <DialogContent>
            ...
            <DialogFooter>
              <Button onClick={deleteProductHandler}>Cancel</Button>
